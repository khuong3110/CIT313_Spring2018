...
class Post extends Model{
    ...
    public function getAllPostsByCat($catID){


        $sql =  'SELECT pID, title, content, date, posts.uID, posts.categoryID, categories.name, first_name, last_name FROM posts INNER JOIN users ON posts.uID = users.uID INNER JOIN categories ON posts.categoryID = categories.categoryID WHERE posts.categoryID = ?';

        // perform query
        $results = $this->db->execute($sql, array($catID));
